...
int main(int argc, char *argv[])
{
    ...
    // 期待値生成
    std::int8_t data0[PIPELINE_SIZE+1][MEM_SIZE];
    std::int8_t data1[PIPELINE_SIZE+1][MEM_SIZE];
    for (int i = 0; i < MEM_SIZE; ++i) {
        data0[0][i] = mem0[i];
        data1[0][i] = mem1[i];
    }
    ...
    for (int i = 0; i < MEM_SIZE; ++i) {
        exp[i] = data0[PIPELINE_SIZE][i] + data1[PIPELINE_SIZE][i];
    }
    ...
    // 結果チェック
    for (int i = 0; i < MEM_SIZE; ++i) {
        std::cout << "MEM[" << i << "] : " << (int)mem2[i] << "  exp : " << (int)exp[i];
        if (mem2[i] != exp[i]) {
