...
int main(int argc, char *argv[])
{
    ...
    std::uint8_t mem0[MEM_SIZE];
    std::uint8_t mem1[MEM_SIZE];
    std::uint8_t mem2[MEM_SIZE];
    std::uint8_t exp[MEM_SIZE];

    // 乱数で初期値生成
    std::random_device seed_gen;
    std::mt19937 engine(seed_gen());
    for (int i = 0; i < MEM_SIZE; ++i) {
        mem0[i] =  (std::uint8_t)engine();
        mem1[i] =  (std::uint8_t)engine();
    }
    ...
    // 結果チェック
    for (int i = 0; i < MEM_SIZE; ++i) {
        std::cout << "MEM[" << i << "] " << (int)mem0[i] << " + "  << (int)mem1[i] << " => " << (int)mem2[i] << "  exp : " << (int)exp[i];
        if (mem2[i] != exp[i]) {
